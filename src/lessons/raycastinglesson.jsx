...
function RaycastingLesson() {
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      if (hoveredObject.current) {
        ...
        setTimeout(() => {
          if (obj) obj.scale.set(originalScale, originalScale, originalScale);
        }, 100);
      }
    };
  ...
  }, [hoverColor, selectColor, containerRef]);

  const codeSnippet = `// Raycaster setup
...
if (intersects.length > 0) {
  ...
  selected.material.color.set('${hoverColor}');
}`;
